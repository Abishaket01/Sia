...
export class QueueWorkflowService {
   ...
   */
  async startHealthCheckSchedule(
    agentId: string
    ...
    try {
      const { agentHealthCheckWorkflow } = await import(
        '../temporal/workflows'
      );
      ...
      console.log(
        `   ✅ Successfully created health check schedule: ${scheduleId}`
      );
      return { scheduleId };
    } catch (error) {
      const errorMessage =
      ...
      if (
        ...
        console.log(
          `   Health check schedule ${scheduleId} was created by another instance`
        );
        return { scheduleId };
      }
      throw error;
